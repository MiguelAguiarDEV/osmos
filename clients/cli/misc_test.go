...
func TestDetectMime(t *testing.T) {
	if got := detectMime("/tmp/a.txt", ""); got != "text/plain" {
		t.Fatalf("txt: %s", got)
	}
	if got := detectMime("a/b/c.png", ""); got != "image/png" {
		t.Fatalf("png: %s", got)
	}
	if got := detectMime("noext", "application/octet-stream"); got != "application/octet-stream" {
